            ...
            if (nCode >= 0 && isMappingEnabled)
            {
                ...
                else if (isSpacePressed && (wParam == (IntPtr)WM_KEYDOWN))
                {
                    ...
                    switch (keyboardStruct.vkCode)
                    {
                        ...
                        case 0x4E: // N键的虚拟键码
                            {
                                INPUT[] shiftHomeInputs = new INPUT[4];
                                
                                shiftHomeInputs[0].type = INPUT_KEYBOARD;
                                shiftHomeInputs[0].input.ki.wVk = VK_LSHIFT;
                                shiftHomeInputs[0].input.ki.dwFlags = 0;

                                shiftHomeInputs[1].type = INPUT_KEYBOARD;
                                shiftHomeInputs[1].input.ki.wVk = VK_HOME;
                                shiftHomeInputs[1].input.ki.dwFlags = 0;

                                shiftHomeInputs[2].type = INPUT_KEYBOARD;
                                shiftHomeInputs[2].input.ki.wVk = VK_HOME;
                                shiftHomeInputs[2].input.ki.dwFlags = KEYEVENTF_KEYUP;

                                shiftHomeInputs[3].type = INPUT_KEYBOARD;
                                shiftHomeInputs[3].input.ki.wVk = VK_LSHIFT;
                                shiftHomeInputs[3].input.ki.dwFlags = KEYEVENTF_KEYUP;

                                SendInput(4, shiftHomeInputs, Marshal.SizeOf(typeof(INPUT)));
                                hasPerformedMapping = true;
                                return (IntPtr)1;
                            }
                        case 0x4D: // M键的虚拟键码
                            {
                                INPUT[] shiftEndInputs = new INPUT[4];
                                
                                shiftEndInputs[0].type = INPUT_KEYBOARD;
                                shiftEndInputs[0].input.ki.wVk = VK_LSHIFT;
                                shiftEndInputs[0].input.ki.dwFlags = 0;

                                shiftEndInputs[1].type = INPUT_KEYBOARD;
                                shiftEndInputs[1].input.ki.wVk = VK_END;
                                shiftEndInputs[1].input.ki.dwFlags = 0;

                                shiftEndInputs[2].type = INPUT_KEYBOARD;
                                shiftEndInputs[2].input.ki.wVk = VK_END;
                                shiftEndInputs[2].input.ki.dwFlags = KEYEVENTF_KEYUP;

                                shiftEndInputs[3].type = INPUT_KEYBOARD;
                                shiftEndInputs[3].input.ki.wVk = VK_LSHIFT;
                                shiftEndInputs[3].input.ki.dwFlags = KEYEVENTF_KEYUP;

                                SendInput(4, shiftEndInputs, Marshal.SizeOf(typeof(INPUT)));
                                hasPerformedMapping = true;
                                return (IntPtr)1;
                            }
